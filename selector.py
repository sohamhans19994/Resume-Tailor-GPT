...
class Selector:
    ...
    def write_resume_points(self,jd_keywords_str, experience_description, example_resume_points):
        generate_experience_bullet_template = f"""
            You identified that the keywords in the job description as follows \n
            {jd_keywords_str}
            Given that your resume has an experience described below:\n
            ...
            First, think of a few keywords that this experience can be aligned to. \n 
            Based on how well the keywords can be aligned, give the experience a relevance score between 1 to 10 with respect to the job description keywords. This score will be used to rank the items in your resume, and all of the experiences are in the AI and SWE space, so be stringent in giving a score to ensure that finally the scores are approximately in a bell curve centered at 5\n
            Finally, write down 2 to 3 resume style points that are tailored to the job such that they use most of the keywords you chose to align. Make sure to use any quantitative results given in the description and examples that may align align to each point\n 
    ...
    def extract_data(self):
        ...
        for exp in experiences:
            resume_points_obj = self.write_resume_points(keywords_str,exp['Experience Description'],exp['Example Points'])
        ...
        for proj in projects:
            resume_points_obj = self.write_resume_points(keywords_str,proj['Project Description'],proj['Example Points'])
